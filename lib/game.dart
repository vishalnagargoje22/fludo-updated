...
class _FludoGameState extends State<FludoGame> with TickerProviderStateMixin {
              ...
              builder: (ctx, snapshot) {
                Offset currentspot = Offset(
                    ...
                    double.parse(snapshot.data.documents
                        ...
                        .toString()));
                data[playerIndex]
                    [pawnIndex.toString()] = [currentspot.dx, currentspot.dy];
                // print(currentspot);
                //
      ...
      for (int pawnIndex = 0;
        ...
        if (_pawnCurrentStepInfo[_currentTurn][pawnIndex]
          ...
          if (clickedPawnIndex == 0) {
            if (_diceOutput == 6) {
              _diceOutput = 1;
              //to move pawn out of the house when 6 is rolled
              List<Map<String, List>> data = [];
              for (int player = 0; player < 4; player++) {
                data.add({
                  '${player.toString()}': [0, 0]
              ...
              data[_currentTurn]
                  ['$pawnIndex'] = [clickOffset.dx, clickOffset.dy];
            } else
  ...
  _movePawn({bool considerCurrentStep = false}) {
    ...
    for (int player = 0; player < 4; player++) {
      for (int pawn = 0; pawn < 4; pawn++) {
        if (player == playerIndex && pawn == pawnIndex)
          data[player]['${pawn.toString()}'] = [
            _playerAnimList[player][pawn].value.dx,
            _playerAnimList[player][pawn].value.dy
          ];
    ...
    // data[_currentTurn][_selectedPawnIndex.toString()] = [
    //   _playerAnimList[_currentTurn][_selectedPawnIndex].value.dx,
    //   _playerAnimList[_currentTurn][_selectedPawnIndex].value.dy
    // ];
    Firestore.instance
        .collection('PlayersPosition')
        .document('oyD87SESFbLlht5POF3w')
